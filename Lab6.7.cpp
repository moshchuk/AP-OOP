...
int main() {
    ...
    sort(result);
    cout << "Пiсля повного сортування:\n" << result;

    sort_if(result, IsNegative());
    cout << "Пiсля сортування лише вiд’ємних:\n" << result;
...
void sort_if(Container& c, Predicate pred) {
    using T = typename Container::value_type;
    std::vector<T> selected;

    for (auto it = c.begin(); it != c.end(); ++it)
        if (pred(*it)) selected.push_back(*it);

    std::sort(selected.begin(), selected.end());

    auto sel_it = selected.begin();
    for (auto it = c.begin(); it != c.end(); ++it)
        if (pred(*it)) *it = *sel_it++;
}
// Array.cpp
#include "Array.h"
